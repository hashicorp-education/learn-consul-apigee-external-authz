...
module "apigee-x-core" {
  source               = "github.com/apigee/terraform-modules//modules/apigee-x-core?ref=v0.17.0"
  project_id           = module.project.project_id
...
module "apigee-x-bridge-mig" {
  ...
  source      = "github.com/apigee/terraform-modules//modules/apigee-x-bridge-mig?ref=v0.17.0"
  project_id  = module.project.project_id
  network     = module.vpc.network.id
  subnet      = module.vpc.subnet_self_links[local.subnet_region_name[each.value.region]]
  region      = each.value.region
  endpoint_ip = module.apigee-x-core.instance_endpoints["instance-${each.key}"]
}

# Module to create L7 Loadbalancer for Apigee Managed Instance Group Backend
module "mig-l7xlb" {
  source          = "github.com/apigee/terraform-modules//modules/mig-l7xlb?ref=v0.17.0"
  project_id      = module.project.project_id
  name            = "apigee-xlb"
...
module "nip-development-hostname" {
  source             = "github.com/apigee/terraform-modules//modules/nip-development-hostname?ref=v0.17.0"
  project_id         = module.project.project_id
